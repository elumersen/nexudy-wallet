  ...
  const getBrandColor = (brand: string) => {
    const colors: Record<string, { gradient: string; overlay: string }> = {
      visa: { 
        ...
        overlay: "bg-gradient-to-br from-blue-500/20 via-transparent to-purple-500/20"
      },
      mastercard: { 
        gradient: "from-slate-700/90 via-slate-600/80 to-slate-700/90",
        overlay: "bg-gradient-to-br from-red-500/20 via-transparent to-orange-500/20"
      },
      amex: { 
        gradient: "from-slate-500/90 via-slate-400/80 to-slate-500/90",
        overlay: "bg-gradient-to-br from-blue-600/20 via-transparent to-indigo-600/20"
      },
      discover: { 
        gradient: "from-slate-600/90 via-slate-500/80 to-slate-600/90",
        overlay: "bg-gradient-to-br from-orange-500/20 via-transparent to-amber-500/20"
      },
      default: { 
        gradient: "from-slate-600/90 via-slate-500/80 to-slate-600/90",
        overlay: "bg-gradient-to-br from-gray-500/20 via-transparent to-gray-600/20"
      }
    };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2].map((i) => (
            <div key={i} className="relative aspect-[1.586/1] rounded-3xl overflow-hidden backdrop-blur-2xl border border-white/20 shadow-xl">
          ...
          <Card className="border-dashed border-2">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 max-w-sm">
                Add a card to make payments faster and easier. Your card details are securely stored.
              </p>
              <Button 
                onClick={() => setShowAddCard(true)}
                variant="outline"
            ...
            {cards.map((card) => {
              ...
              return (
                <div key={card.id} className="relative group">
                  ...
                  >
                    {/* Color Overlay */}
                    <div className={cn("absolute inset-0", brandColors.overlay)}></div>
